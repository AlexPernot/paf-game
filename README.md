...
To run the game, you need a `data/game.csv`. Here's how to regenerate it if you need to have up-to-date data:
...
### parse-popularity.js
Reads into the data folder's `tmdb-movies.json` file to add the popularity column to `data/game.csv`. This file is too big for git and has to be sourced here (replace the MM_DD_YYYY by yesterday's date): https://files.tmdb.org/p/exports/movie_ids_MM_DD_YYYY.json.gz
